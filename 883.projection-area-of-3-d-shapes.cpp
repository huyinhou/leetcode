/*
 * @lc app=leetcode id=883 lang=cpp
 *
 * [883] Projection Area of 3D Shapes
 *
 * https://leetcode.com/problems/projection-area-of-3d-shapes/description/
 *
 * algorithms
 * Easy (65.37%)
 * Total Accepted:    16.4K
 * Total Submissions: 25.1K
 * Testcase Example:  '[[2]]'
 *
 * On a N * N grid, we place some 1 * 1 * 1 cubes that are axis-aligned with
 * the x, y, and z axes.
 * 
 * Each value v = grid[i][j] represents a tower of v cubes placed on top of
 * grid cell (i, j).
 * 
 * Now we view the projection of these cubes onto the xy, yz, and zx planes.
 * 
 * A projection is like a shadow, that maps our 3 dimensional figure to a 2
 * dimensional plane. 
 * 
 * Here, we are viewing the "shadow" when looking at the cubes from the top,
 * the front, and the side.
 * 
 * Return the total area of all three
 * projections.
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * Example 1:
 * 
 * 
 * Input: [[2]]
 * Output: 5
 * 
 * 
 * 
 * Example 2:
 * 
 * 
 * Input: [[1,2],[3,4]]
 * Output: 17
 * Explanation: 
 * Here are the three projections ("shadows") of the shape made with each
 * axis-aligned plane.
 * 
 * 
 * 
 * 
 * Example 3:
 * 
 * 
 * Input: [[1,0],[0,2]]
 * Output: 8
 * 
 * 
 * 
 * Example 4:
 * 
 * 
 * Input: [[1,1,1],[1,0,1],[1,1,1]]
 * Output: 14
 * 
 * 
 * 
 * Example 5:
 * 
 * 
 * Input: [[2,2,2],[2,1,2],[2,2,2]]
 * Output: 21
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * Note:
 * 
 * 
 * 1 <= grid.length = grid[0].length <= 50
 * 0 <= grid[i][j] <= 50
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 * 
 */

// #define LEETCODE
#ifdef LEETCODE
#include <cassert>
#include <cstddef>
#include <cstdio>
#include <string>
#include <ostream>
#include <iterator>
#include <iostream>
#include <vector>
#include <iterator>
#include <algorithm>

using std::vector;
using std::string;
using std::ostringstream;
using std::cout;
#endif
#include <numeric>

class Solution {
public:
    int projectionArea(vector<vector<int>>& grid) {
        vector<int> maxr(grid.size());
        vector<int> maxc(grid.size());
        int area = 0;
        for (size_t r = 0; r < grid.size(); r++) {
            vector<int> &tmp = grid[r];
            for (size_t c = 0; c < grid.size(); c++) {
                if (tmp[c] > 0) {
                    area++;
                }
                if (tmp[c] > maxr[r]) {
                    maxr[r] = tmp[c];
                }
                if (tmp[c] > maxc[c]) {
                    maxc[c] = tmp[c];
                }
            }
        }
        return area + std::accumulate(maxr.begin(), maxr.end(), 0) 
            + std::accumulate(maxc.begin(), maxc.end(), 0);
    }
};

#ifdef LEETCODE
int main(int argc, char *argv[]) {
    Solution s;
    vector<vector<int> > g1 = {
        vector<int>{2}
    };
    assert(5 == s.projectionArea(g1));
    vector<vector<int> > g2 = {
        {1,2},
        {3,4}
    };
    assert(17 == s.projectionArea(g2));
    vector<vector<int> > g3 = {
        {1,0},
        {0,2}
    };
    assert(8 == s.projectionArea(g3));
    vector<vector<int> > g4 = {
        {1,1,1},
        {1,0,1},
        {1,1,1}
    };
    assert(14 == s.projectionArea(g4));
    return 0;
}
#endif
